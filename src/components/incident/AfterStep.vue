...
<template>
  <section class="section" :class="{ invalidSection: fieldErrors['incident.afterSection'] }">
    ...
    <div class="field-grid">
      ...
      <label v-if="hasOther(form.incident.after, form.incident.afterOther)" class="field full"><span class="field-label">{{ formLabels.incident.afterOther }} <span v-if="form.incident.after.includes('Inne')" class="required-mark">*</span></span><input class="text-input" :class="{ invalid: fieldErrors['incident.afterOther'] }" v-model="form.incident.afterOther" /><span v-if="fieldErrors['incident.afterOther']" class="field-error">{{ fieldErrors['incident.afterOther'] }}</span></label>
