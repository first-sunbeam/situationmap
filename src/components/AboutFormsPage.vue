  ...
  <section class="panel about-page">
    <div class="about-layout">
      <nav class="about-nav" :aria-label="content.navTitle">
        ...
        <a v-for="section in content.sections" :key="section.id" :href="`#${section.id}`">
          {{ section.title }}
        </a>
      </nav>

      <div class="about-content">
        <section v-for="section in content.sections" :id="section.id" :key="section.id" class="about-section">
          <span v-if="section.badge" class="about-badge">{{ section.badge }}</span>
          <h2>{{ section.title }}</h2>
          ...
          <div v-if="section.cards" class="about-cards">
            <article v-for="card in section.cards" :key="card.title" class="about-card">
              <h3>{{ card.title }}</h3>
              <p>{{ card.text }}</p>
            </article>
          </div>
            ...
            <table class="about-table">
              <thead>
                <tr>
                  ...
                  <th>{{ language === "en" ? "What it captures" : "Co mierzy" }}</th>
                  <th>{{ language === "en" ? "Why it matters" : "Po co to jest" }}</th>
                </tr>
              </thead>
              <tbody>
