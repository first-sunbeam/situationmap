...
describe("budowanie wiadomości e-mail", () => {
  it("buduje temat i treść dla formularza prostego", () => {
    ...
    expect(email.body).toContain(
      "3. Przebieg sytuacji – co można było zaobserwować?: Protest, płacz i odmowa przejścia do kolejnej aktywności.",
    );
  ...
  it("zawiera pola incydentu w trybie rozszerzonej karty zdarzenia", () => {
    ...
    expect(email.body).toContain(
      "Poziom napięcia przed zdarzeniem: 1 podwyższony",
    );
  ...
  it("zawiera mapę w trybie mapy i pomija sekcję incydentu", () => {
    ...
    expect(email.body).toContain(
      "W jakich miejscach osoba najchętniej przebywa?: Pokój osoby",
    );
  ...
  it("buduje angielską treść i tłumaczy wartości checkboxów", () => {
    ...
    expect(email.body).toContain(
      "Where does the person prefer staying?: Person's room",
    );

    expect(email.body).toContain(
      "What helps reduce tension?: Silence / reducing stimuli",
    );
